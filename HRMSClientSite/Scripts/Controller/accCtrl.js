...
app.controller("accCtrl", function ($q, $scope, $http, $window, $rootScope, $cookies,accSvc) {
    var serviceBaseUrl = "http://localhost:33306/";
    ...
    $scope.login = function () {
        var obj = { 'username': $scope.lg.Email, 'password': $scope.lg.UserPassword, 'grant_type': 'password' };
    ...
    $scope.getUserData = function () {
        $http.get(serviceBaseUrl + "odata/EmployeeLogins(" + $cookies.get('uid') + ")"
        ).then(function (response) {
